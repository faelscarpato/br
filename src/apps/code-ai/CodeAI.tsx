...
const ACTIONS = {
  ...
  refatorar:
    "Refatore o código abaixo para legibilidade e boas práticas. Retorne o código refatorado e um pequeno comentário do que mudou:",
};
  ...
  async function run(action: keyof typeof ACTIONS) {
    ...
    const r = await callAI(provider, {
      ...
      messages: [
        {
          role: "system",
          content: "Você é um assistente de programação sênior. Use markdown e blocos de código.",
        },
  ...
  return (
    ...
    >
      <div className="grid h-full grid-cols-1 gap-3 p-3 md:grid-cols-2">
        ...
        />
        <pre
          className="h-full min-h-[200px] overflow-auto rounded-lg border border-[color:var(--glass-border)] bg-card/60 p-3 font-mono text-xs whitespace-pre-wrap"
          style={{ fontFamily: "var(--font-mono)" }}
        >
