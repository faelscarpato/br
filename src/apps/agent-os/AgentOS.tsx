...
export function AgentOS() {
  ...
  function startNew() {
    ...
    if (!p) {
      notify({ title: "Configure um provider antes", variant: "warning" });
      return;
    }
  ...
  async function send() {
    ...
    setMessages((m) => ({
      ...m,
      [selected.id]: [...next, { role: "assistant", content: r.text }],
    }));
    ...
    <div className="grid h-full grid-cols-1 md:grid-cols-[260px_1fr]">
      <aside className="flex flex-col border-r border-[color:var(--glass-border)]">
        ...
        <div className="flex-1 overflow-auto px-2 pb-2">
          {agents.length === 0 && (
            <p className="px-2 text-xs text-muted-foreground">Crie seu primeiro agente.</p>
          )}
          {agents.map((a) => (
            <button
              key={a.id}
              onClick={() => setSelectedId(a.id)}
              className={`mb-1 block w-full rounded-md px-3 py-2 text-left text-sm transition ${selectedId === a.id ? "bg-secondary" : "hover:bg-secondary/60"}`}
            >
        ...
        {editing && draft ? (
          <div className="flex flex-1 flex-col gap-3 overflow-auto p-4">
            ...
            <textarea
              ...
              placeholder="Instruções (system prompt)"
              className="min-h-[140px] resize-none rounded-md bg-secondary p-2 text-sm outline-none"
            />
            ...
            <div className="flex gap-2">
              ...
              {agents.some((a) => a.id === draft.id) && (
                <button
                  ...
                  className="ml-auto rounded-md bg-destructive px-3 py-2 text-sm text-destructive-foreground"
                >
                  Excluir
                </button>
              )}
          ...
          <>
            <div className="flex items-center justify-between border-b border-[color:var(--glass-border)] px-4 py-2">
              <div>
                ...
                <div className="text-xs text-muted-foreground">
                  {provider?.name} · {selected.model}
                </div>
              </div>
              <button onClick={startEdit} className="rounded-md bg-secondary px-3 py-1 text-xs">
                Editar
              </button>
            </div>
            <div ref={scrollRef} className="flex-1 space-y-3 overflow-auto p-4">
              {(messages[selected.id] ?? []).map((m, i) => (
                <div
                  key={i}
                  className={`max-w-[80%] rounded-2xl px-3 py-2 text-sm whitespace-pre-wrap ${m.role === "user" ? "ml-auto bg-primary text-primary-foreground" : "bg-secondary"}`}
                >
                  {m.content}
                </div>
              ))}
              {(messages[selected.id] ?? []).length === 0 && (
                <p className="text-xs text-muted-foreground">
                  Inicie uma conversa com {selected.name}.
                </p>
              )}
            ...
            <div className="flex gap-2 border-t border-[color:var(--glass-border)] p-3">
              <input
                ...
                className="flex-1 rounded-md bg-secondary p-2 text-sm outline-none"
              />
              <button
                onClick={send}
                disabled={loading}
