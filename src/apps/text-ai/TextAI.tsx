...
const PROMPTS = {
  ...
  revisar:
    "Revise o texto a seguir, corrigindo gramática, clareza e fluidez. Retorne apenas o texto corrigido:",
      ...
      messages: [
        {
          role: "system",
          content:
            "Você é um assistente de escrita preciso, conciso e em português brasileiro quando aplicável.",
        },
      ...
      <div className="grid h-full grid-cols-1 gap-3 p-3 md:grid-cols-[1fr_320px]">
        ...
        <aside className="flex flex-col gap-2 rounded-lg bg-secondary/40 p-3">
          ...
          <div className="flex-1 space-y-2 overflow-auto">
            {history.length === 0 && (
              <p className="text-xs text-muted-foreground">Suas interações aparecerão aqui.</p>
            )}
            {history.map((h, i) => (
              <button
                key={i}
