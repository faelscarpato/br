...
export function TerminalAI() {
  ...
  const [lines, setLines] = useState<Line[]>([
    { kind: "out", text: "BrOS Terminal IA — digite um comando ou pergunta. (`help` para dicas)" },
  ]);
  ...
  async function submit() {
    ...
    if (cmd.trim() === "help") {
      setLines((l) => [
        ...
        {
          kind: "out",
          text: "Você pode pedir explicações de comandos, gerar scripts, simular saídas. Ex: `explique tar -xzvf` ou `gere um bash que renomeia .jpg para .png`",
        },
      ]);
      return;
    }
      ...
      messages: [
        {
          role: "system",
          content:
            "Você é um terminal explicativo. Não execute nada de verdade. Responda de forma concisa, em texto monoespaçado, simulando saída quando aplicável.",
        },
  ...
  return (
    <div
      className="flex h-full flex-col bg-[oklch(0.1_0.02_250)] p-3 font-mono text-xs text-green-300"
