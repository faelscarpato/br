...
export function VisionAI() {
  ...
  useEffect(() => {
    try {
      setItems(JSON.parse(localStorage.getItem(STORE) || "[]"));
    } catch {
      /* noop */
    }
  }, []);
      ...
      messages: [
        {
          role: "system",
          content:
            "Você é um diretor de arte. Gere um conceito visual detalhado e um prompt otimizado para modelos de imagem (Midjourney/Stable Diffusion), em português. Estruture: Conceito, Composição, Cores, Iluminação, Prompt Final.",
        },
        ...
        <aside className="flex flex-col gap-3">
          <div className="flex flex-col gap-2 rounded-lg bg-secondary/40 p-3">
            ...
            <textarea
              ...
              placeholder="ex: paisagem brasileira ao entardecer..."
              className="min-h-[80px] resize-none rounded-md bg-card/60 p-2 text-sm outline-none"
            />
            <label className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">
              Estilo
            </label>
            <input
              ...
              className="rounded-md bg-card/60 p-2 text-sm outline-none"
            />
            <button
              onClick={generate}
              disabled={loading}
              className="mt-2 rounded-md bg-primary py-2 text-sm font-medium text-primary-foreground disabled:opacity-50"
            >
              {loading ? "Gerando conceito..." : "Gerar conceito"}
            </button>
            <p className="text-[10px] text-muted-foreground">
              Geração de imagem requer um provider compatível. Este app entrega conceito + prompt
              otimizado.
            </p>
          </div>
          <div className="flex-1 overflow-auto rounded-lg bg-secondary/40 p-2">
            <h4 className="px-1 py-1 text-xs font-semibold uppercase tracking-wider text-muted-foreground">
              Galeria
            </h4>
            <div className="grid grid-cols-2 gap-2">
              {items.map((it) => (
                <button
                  ...
                  className="aspect-square rounded-md p-2 text-left text-[10px] [background:var(--gradient-br)] text-black/80 hover:opacity-90"
                >
                  <div className="line-clamp-4 font-medium">{it.prompt}</div>
                </button>
              ))}
              {items.length === 0 && (
                <p className="col-span-2 px-1 text-xs text-muted-foreground">Sem itens ainda.</p>
              )}
            </div>
          </div>
        </aside>
        <section className="rounded-lg border border-[color:var(--glass-border)] bg-card/60 p-4 overflow-auto">
          {selected ? (
            ...
            <p className="text-sm text-muted-foreground">
              Selecione um item ou gere um novo conceito visual.
            </p>
          )}
        </section>
      </div>
