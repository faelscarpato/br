...
export function WindowManagerProvider({ children }: { children: ReactNode }) {
  ...
  const focus = useCallback((id: string) => {
    ...
    setWindows((ws) =>
      ws.map((w) =>
        w.id === id
          ? { ...w, z: zCounter, state: w.state === "minimized" ? "normal" : w.state }
          : w,
      ),
    );
    setFocusId(id);
  ...
  const restore = useCallback((id: string) => {
    zCounter += 1;
    setWindows((ws) => ws.map((w) => (w.id === id ? { ...w, state: "normal", z: zCounter } : w)));
    setFocusId(id);
  }, []);
  ...
  const toggleMinimize = useCallback(
    (id: string) => {
      ...
      if (!w) return;
      if (w.state === "minimized" || focusId !== id) {
        restore(id);
      } else {
        minimize(id);
      }
    },
    [windows, focusId, restore, minimize],
  );
  ...
  const resize = useCallback(
    (id: string, width: number, height: number, x?: number, y?: number) => {
      setWindows((ws) =>
        ws.map((w) => (w.id === id ? { ...w, width, height, x: x ?? w.x, y: y ?? w.y } : w)),
      );
    },
    [],
  );
