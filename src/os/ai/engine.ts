...
export async function callAI(
  ...
  if (!provider)
    return { ok: false, error: "Nenhum provider de IA selecionado. Abra IA Settings." };
    ...
    if (!res.ok) {
      ...
      if (res.status === 429) {
        return {
          ...
          code: 429,
          error:
            "Este provider atingiu o limite de uso. Tente novamente em instantes ou selecione outro.",
        };
      }
      if (res.status === 401 || res.status === 403) {
        return {
          ok: false,
          code: res.status,
          error: "Falha de autenticação. Verifique sua chave de API em IA Settings.",
        };
      }
      return {
        ok: false,
        code: res.status,
        error: `Erro ${res.status}: ${text.slice(0, 240) || res.statusText}`,
      };
    }
